...
void
calc_md5(unsigned char *output, unsigned char *input, unsigned int inlen)  
{
	MD5_CTX context;
...
std::string 
convert_hex_digest(unsigned char *input)
{
	char output[33] = {0};
	for (int i = 0; i < 16; ++i) 
	{
		snprintf(reinterpret_cast<char*>(&(output[i*2])), 16*2, "%02x", (unsigned int)input[i]);
	}
	return std::string(output);
}

std::string 
calc_md5_string(unsigned char *input, unsigned int inlen)
{
	unsigned char outmd5hex[16];
	calc_md5(outmd5hex, input, inlen);
	return convert_hex_digest(outmd5hex);
}
